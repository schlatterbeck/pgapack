...
\begin{figure}
\centerline{\includegraphics{refpoints.eps}}
...
\begin{sloppypar}
\verb+PGASetPrintFrequencyValue(ctx,40)+ will print population statistics every
...
of the function \verb+PGASetPrintOptions+ to \verb+PGA_REPORT_ONLINE+,
\verb+PGA_REPORT_OFFLINE+, \verb+PGA_REPORT_WORST+, \verb+PGA_REPORT_AVERAGE+,
\verb+PGA_REPORT_GENE_DISTANCE+, or \verb+PGA_REPORT_STRING+ to print the online
analysis, offline analysis, worst evaluation, average evaluation, genetic
distance, or string itself, respectively.  \verb+PGASetPrintOptions+ may be
...
PGAGeneDistance(ctx,pop)} returns a {\tt double}, which is the average
genetic distance between the strings in population {\tt pop}.
The function call
\begin{verbatim}
...
difference of two individuals. It is used when restricted
tournament selection is in use. In addition it is used when reporting of
genetic distance is selected by calling \verb+PGASetPrintOptions+ with
\verb+PGA_REPORT_GENE_DISTANCE+.  There are implementations for the
...
\item Report Options
\begin{itemize}
\item \verb+PGA_REPORT_ONLINE+
\item \verb+PGA_REPORT_OFFLINE+
\item \verb+PGA_REPORT_GENE_DISTANCE+
\item \verb+PGA_REPORT_STRING+
\item \verb+PGA_REPORT_WORST+
\item \verb+PGA_REPORT_AVERAGE+
\end{itemize}
...
double &  PGAGeneDistance(PGAContext *ctx, int popindex) \\ \hline
