...
The two replacement types \textit{pairwise best} and \textit{restricted
tournament replacement} have selection pressure in addition to providing
...
participant in the tournament) is used.
...
\begin{figure}
\centerline{\includegraphics{generation.eps}}
...
tournament (size = 2). Note that with a tournament size of 1 (or with
...
specified by \verb+PGASetDECrossoverProb+ (default
\DefaultDECrossoverProb) selects an
element from either the donor vector $V_i$ or $X_i$. At least one random
element from $V_i$ is always selected, so with a crossover probability
of 0, exactly one element from the donor vector is selected. With a
crossover probability of 1, \textit{all} elements from the donor vector
are selected. The resulting individual after crossover is placed in the
...
So for the default DE strategy the name is \textit{DE/rand/1/bin}.
Currently only this crossover variant is implemented.
...
$0.5 * (F + 1)$ \cite[p.~118]{PSL05} where F is the scale factor, it can be
set with \verb+PGASetDEAuxFactor+. Note that the either-or variant
ignores the parameter specifying the number of difference vectors
(specified with \verb+PGASetDENumDiffs+). The expression rand$_i$(0,~1)
denotes a random number in the range $[0, 1)$ which is re-generated for
each individual $i$.
...
\textit{dither} (and is currently not implemented in \pga) while the
...
index $j$ denoting the allele \cite[p.~80]{PSL05}:

\begin{equation}\label{jitter}
\begin{array}{ll}
F_j = (F + d \cdot (\mbox{rand}_j(0, 1) - 0.5), & d < 2 \cdot F \\
\end{array}
\end{equation}

The new $F_j$ replaces $F$ in
equations~\ref{de/rand/1}--\ref{de/either-or/1} and defaults to
\DefaultDEJitter. It can be set with \verb+PGASetDEJitter+.
...
digital filter design \cite[p.~440]{PSL05}.
