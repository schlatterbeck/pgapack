...
optimized separately) a low crossover rate $0 \le Cr \le 0.2$ is a good
choice. For non-decomposable problem a high crossover rate should be
chosen, i.e. $0.9 \le Cr \le 1$ \cite[p.~98]{PSL05}.
...
\textit{Pareto Front} after the mathematicial Vilfredo Pareto who first
defined the concept today known as \textit{pareto optimality}. \pga\
currently does not implement a multiobjective algorithm.
...
auxiliary evaluations as the second paramter. The default is 0.
...
Evaluation compare   & \verb+PGA_USERFUNCTION_EVAL_COMPARE+ \\
\hline
\end{tabular}
\end{table}
...
The evaluation comparison function can be used to re-define how
evaluations are compared. Currently with constrained optimization, the
function uses the auxiliary evaluations together with the objective
function value. For new algorithms this can be redefined to provide new
functionality.
