...
A last algorithm is typically used for multi-objective optimization with
the Nondominated Sorting Genetic Algorithm (Version~2), NSGA-II
...
by setting \verb+PGASetSumConstraintsFlag+ to \verb+PGA_TRUE+. NSGA-II
is the only possible population replacement algorithm when using
...
For NSGA-II replacement \verb+PGA_POPREPL_NSGA_II+ is used. The number
...
If the difference between the two is $>0$, these auxiliary evaluations are
used for multi-objective optimization. Only the NSGA-II replacement is
