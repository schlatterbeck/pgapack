...
PGAHash      HashString       (PGAContext *, int, int);
...
int main (int argc, char **argv)
{
    ...
    PGASetUserFunction
        (ctx, PGA_USERFUNCTION_MUTATION,      (void *)Mutation);
    PGASetUserFunction
        (ctx, PGA_USERFUNCTION_CROSSOVER,     (void *)Crossover);
    PGASetUserFunction
        (ctx, PGA_USERFUNCTION_PRINTSTRING,   (void *)WriteString);
    PGASetUserFunction
        (ctx, PGA_USERFUNCTION_COPYSTRING,    (void *)CopyString);
    PGASetUserFunction
        (ctx, PGA_USERFUNCTION_DUPLICATE,     (void *)DuplicateString);
    PGASetUserFunction
        (ctx, PGA_USERFUNCTION_HASH,          (void *)HashString);
    PGASetUserFunction
        (ctx, PGA_USERFUNCTION_BUILDDATATYPE, (void *)BuildDT);
    PGASetUp            (ctx);
    PGARun              (ctx, Evaluate);
    PGADestroy          (ctx);
    return 0;
...
/*****************************************************************************
 * Hashing produces a hash over a gene.                                      *
 * Needed for duplicate checking in addition to DuplicateString above.       *
 *****************************************************************************/
